...
export const rideHailingProviderInstances = pgTable(
  "ride_hailing_provider_instances",
  {
    ...
    status: text("status")
      .$type<RideHailingProviderInstanceStatus>()
      .notNull()
      .default("ACTIVE"),
  ...
  (table) => ({
    providerTypeStatusIdx: index(
      "ride_hailing_provider_instances_type_status_idx",
    ).on(table.providerType, table.status),
    providerInstanceUnique: uniqueIndex(
      "ride_hailing_provider_instances_type_key_unique",
    ).on(table.providerType, table.instanceKey),
  }),
);

export type RideHailingProviderInstance =
  typeof rideHailingProviderInstances.$inferSelect;
export type NewRideHailingProviderInstance =
  typeof rideHailingProviderInstances.$inferInsert;
