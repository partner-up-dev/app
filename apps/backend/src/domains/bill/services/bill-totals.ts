...
export const sumBillLineAmounts = (lines: BillLine[], kind: BillLineKind): number =>
  lines.filter((line) => line.kind === kind).reduce((sum, line) => sum + line.amountFen, 0);

export const getBillChargeTotal = (bill: Bill): number => sumBillLineAmounts(bill.lines, "CHARGE");

export const getBillRefundTotal = (bill: Bill): number => sumBillLineAmounts(bill.lines, "REFUND");
