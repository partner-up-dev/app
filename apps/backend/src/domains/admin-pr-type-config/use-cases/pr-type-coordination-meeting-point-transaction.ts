...
import { toPRTypeConfigOperatorDetail } from "../../pr-type-config/services/projection";
import type { AdminPRTypeConfigCoordination, AdminPRTypeConfigDetail } from "../contracts";
...
export type PRTypeCoordinationMeetingPointTransactionPort = {
  update(input: {
    type: string;
...
/**
 * Admin PR-type coordination's atomic meeting-point bridge. It keeps source
 * locking, effective-point observation and source event identity on the
 * source side while Notification retains recipient policy and Job mechanics.
 */
export const createPRTypeCoordinationMeetingPointTransactionPort = (
  dependencies: {
    createNotificationPort?: CreateMeetingPointUpdatedNotificationPort;
  } = {},
): PRTypeCoordinationMeetingPointTransactionPort => {
  const createNotificationPort =
    dependencies.createNotificationPort ??
    createTransactionBoundMeetingPointUpdatedNotificationPort;

  return {
    async update(input): Promise<AdminPRTypeConfigDetail> {
