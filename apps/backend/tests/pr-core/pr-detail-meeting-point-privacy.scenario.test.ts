...
import type { EffectiveMeetingPoint } from "../../src/domains/pr-core/services/meeting-point.service";
import type { PRStatus } from "../../src/entities";
import { expectJsonResponse, requestJson } from "../_infra/http/backend-app";
import { scenario } from "../_infra/scenario/scenario";
import { configurePRMeetingPoint, configurePRStatus } from "./_kit/actions/system-state";
...
scenario("active_pr_meeting_point_is_private_to_active_participants", async (ctx) => {
  ...
  assert.equal(activeParticipantDetail.core.meetingPointVisibility, "VISIBLE");
  assert.equal(activeParticipantDetail.core.meetingPoint?.description, "Court 3 glass door");
  assert.equal(
  ...
  assert.equal(outsiderDetail.core.meetingPointVisibility, "ACTIVE_PARTICIPANTS_ONLY");
