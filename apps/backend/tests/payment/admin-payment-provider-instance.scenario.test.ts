...
scenario("admin_payment_provider_instance_create_list_update_preserves_secrets", async () => {
  ...
  const createResponse = await requestJson("/api/admin/payment/provider-instances", {
    method: "POST",
    token: admin.token,
  ...
  const created = await expectJsonResponse<AdminPaymentProviderInstanceResponse>(
    createResponse,
    200,
  );
  ...
  assert.equal(created.config.merchantCertificate.privateKeyPemConfigured, true);
  assert.equal("privateKeyPem" in created.config.merchantCertificate, false);
  assert.equal("certificatePem" in created.config.merchantCertificate, false);

  const storedAfterCreate = await providerRepo.findById(created.id as PaymentProviderInstanceId);
  ...
  const workspaceResponse = await requestJson("/api/admin/payment/provider-instances/workspace", {
    method: "GET",
    token: admin.token,
  });
  const workspace = await expectJsonResponse<AdminPaymentProviderWorkspaceResponse>(
    workspaceResponse,
    200,
  );
  ...
  assert.equal("privateKeyPem" in listed.config.merchantCertificate, false);
  assert.equal(listed.config.platformCertificates.count, 1);
  assert.equal("certificatePem" in listed.config.platformCertificates.items[0]!, false);

  const updateResponse = await requestJson(`/api/admin/payment/provider-instances/${created.id}`, {
    method: "PATCH",
    token: admin.token,
    body: {
  ...
  const updated = await expectJsonResponse<AdminPaymentProviderInstanceResponse>(
    updateResponse,
    200,
  );
  ...
  const storedAfterUpdate = await providerRepo.findById(created.id as PaymentProviderInstanceId);
  assert.ok(storedAfterUpdate);
  assert.equal(storedAfterUpdate.config.apiV3Key, "0123456789abcdef0123456789abcdef");
  assert.equal(
...
scenario(
  "admin_payment_provider_instance_allows_multiple_active_providers_for_same_client",
  async () => {
    const admin = await givenAdminUser("payment-provider-admin-conflict");
    const clientId = `web-${randomUUID()}`;
    const firstCertificate = generateRsaPemPair();
    const secondCertificate = generateRsaPemPair();

    const firstResponse = await requestJson("/api/admin/payment/provider-instances", {
      method: "POST",
      token: admin.token,
      body: buildProviderPayload({
        clientId,
    ...
    const firstProvider = await expectJsonResponse<AdminPaymentProviderInstanceResponse>(
      firstResponse,
      200,
    );

    const secondResponse = await requestJson("/api/admin/payment/provider-instances", {
      method: "POST",
      token: admin.token,
      body: buildProviderPayload({
        clientId,
        ...
        mchId: `1900000001-${randomUUID()}`,
        apiV3Key: "0123456789abcdef0123456789abcdef",
        privateKeyPem: secondCertificate.privateKeyPem,
        certificatePem: null,
      }),
    });

    const secondProvider = await expectJsonResponse<AdminPaymentProviderInstanceResponse>(
      secondResponse,
      200,
    );

    assert.equal(firstProvider.clientId, clientId);
    assert.equal(secondProvider.clientId, clientId);
    assert.equal(firstProvider.status, "ACTIVE");
    assert.equal(secondProvider.status, "ACTIVE");
    assert.notEqual(firstProvider.id, secondProvider.id);
    assert.notEqual(firstProvider.instanceKey, secondProvider.instanceKey);
    ...
    const listedProviderIds = new Set(
      workspace.providerInstances
        .filter((providerInstance) => providerInstance.clientId === clientId)
        .map((providerInstance) => providerInstance.id),
    );
    assert.deepEqual(listedProviderIds, new Set([firstProvider.id, secondProvider.id]));
  },
);

scenario("admin_payment_provider_instance_rejects_invalid_wechatpay_private_key", async () => {
  const admin = await givenAdminUser("payment-provider-admin-invalid-key");

  const response = await requestJson("/api/admin/payment/provider-instances", {
    method: "POST",
    token: admin.token,
    body: buildProviderPayload({
