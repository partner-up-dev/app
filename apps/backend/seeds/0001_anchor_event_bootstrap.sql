...
--
-- Keep this file to configurable baseline records: event pools, POIs,
...
begin
  ...
  insert into anchor_events (
    ...
    beta_group_qr_code,
    pr_creation_policy,
    full_pr_expansion_policy,
    pr_time_window_editor_default_mode,
    status,
    created_at,
    updated_at
  )
  ...
  set
    ...
    default_pr_notes = excluded.default_pr_notes,
    ...
    pr_creation_policy = excluded.pr_creation_policy,
    full_pr_expansion_policy = excluded.full_pr_expansion_policy,
    pr_time_window_editor_default_mode = excluded.pr_time_window_editor_default_mode,
    status = excluded.status,
    updated_at = excluded.updated_at;

  insert into anchor_event_preference_tags (
    anchor_event_id,
    label,
    description,
    status,
  ...
  from (
    values
      (1::bigint, '节奏:新手友好'::text, ''::text, 'PUBLISHED'::text),
      (1::bigint, '节奏:强度适中'::text, ''::text, 'PUBLISHED'::text),
      (2::bigint, '学科:英语'::text, ''::text, 'PUBLISHED'::text),
      (2::bigint, '学科:数学'::text, ''::text, 'PUBLISHED'::text),
      (2::bigint, '状态:互相监督'::text, ''::text, 'PUBLISHED'::text)
  ) as baseline(anchor_event_id, label, description, status)
  where exists (
    select 1
      from anchor_events
     where anchor_events.id = baseline.anchor_event_id
  )
    and not exists (
      select 1
        from anchor_event_preference_tags existing
       where existing.anchor_event_id = baseline.anchor_event_id
