  ...
  <AdminPageScaffold class="admin-analytics-page">
    ...
    <template #main>
      <div class="analytics-dashboard" data-testid="admin-analytics.dashboard">
        ...
        />
        <BIOverviewSurface v-else :view-model="viewModel" />
      </div>
    </template>
  </AdminPageScaffold>
</template>
...
import BIOverviewSurface from "@/domains/analytics/ui/surfaces/BIOverviewSurface.vue";
import { useAnalyticsDashboard } from "@/domains/analytics/use-cases/useAnalyticsDashboard";

const { t } = useI18n();
const { logout } = useAdminAccess();
const viewModel = useAnalyticsDashboard("overview");
