<template>
  <AdminPageScaffold class="admin-analytics-page">
    ...
    <template #rail
      ><AnalyticsFilterRail
        :draft="draft"
        :show-discovery-dimensions="true"
    ...
    <template #main>
      <div class="analytics-dashboard" data-testid="admin-analytics.dashboard">
        ...
        />
        <PRDiscoverySurface v-else :view-model="viewModel" />
      </div>
    </template>
  </AdminPageScaffold>
</template>
...
import PRDiscoverySurface from "@/domains/analytics/ui/surfaces/PRDiscoverySurface.vue";
import { useAnalyticsDashboard } from "@/domains/analytics/use-cases/useAnalyticsDashboard";
const { t } = useI18n();
const { logout } = useAdminAccess();
const viewModel = useAnalyticsDashboard("pr-discovery");
