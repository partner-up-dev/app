<template>
  <article
    class="pr-discovery-demand-card"
    :class="{
      'pr-discovery-demand-card--dragging': isDragging,
      'pr-discovery-demand-card--pending': isPendingState,
      'pr-discovery-demand-card--preview': preview,
      'pr-discovery-demand-card--interactive': isDetailActionAvailable,
    }"
  ...
  >
    <div
      v-if="coverImage"
      class="pr-discovery-demand-card__cover pr-discovery-demand-card__cover--image"
    >
      <PuImg
        class="pr-discovery-demand-card__cover-image"
        :src="coverImage"
        alt=""
        mode="aspectFill"
        :show-loading="false"
      />
      <PuTag
        class="pr-discovery-demand-card__location-tag"
    ...
    <div class="pr-discovery-demand-card__body">
      <section class="pr-discovery-demand-card__primary">
        <p class="pr-discovery-demand-card__time">{{ timeLabel }}</p>
        <PuChipGroup
          v-if="preferenceTags.length > 0"
          class="pr-discovery-demand-card__preference-list"
          gap="sm"
        >
          <PuChip
        ...
        <p v-if="displayNotes" class="pr-discovery-demand-card__notes">
          {{ displayNotes }}
        </p>
      </section>
    </div>
...
import {
  createPRDiscoveryCardSwipePreviewState,
  PR_DISCOVERY_CARD_EXIT_TRANSITION,
  PR_DISCOVERY_CARD_REBOUND_TRANSITION,
  type PRDiscoveryCardSwipePreviewPhase,
  type PRDiscoveryCardSwipePreviewPivotCorner,
...
type SwipePhase = PRDiscoveryCardSwipePreviewPhase;
...
const emit = defineEmits<{
  skip: [];
  "view-detail": [];
  "swipe-preview": [previewState: PRDiscoveryCardSwipePreviewState];
}>();
...
const settleTransition = ref(PR_DISCOVERY_CARD_REBOUND_TRANSITION);
const swipePhase = ref<SwipePhase>("idle");
const pendingAction = ref<SwipeAction | null>(null);
const hasDispatchedExitAction = ref(false);
const previewPivotViewportY = ref<number | null>(null);
const previewPivotCorner = ref<PRDiscoveryCardSwipePreviewPivotCorner | null>(null);
...
const isDetailActionAvailable = computed(
  () => !props.preview && (props.actionAvailable ?? props.detailPrId !== null),
);
...
const cardStyle = computed(() => {
  if (props.preview) {
    ...
    const previewOpacity = Math.max(1 - (depth - 1) * PREVIEW_OPACITY_STEP, PREVIEW_MIN_OPACITY);
...
const resolveSwipeThreshold = (): number => {
  ...
  return Math.max(MIN_DISTANCE_THRESHOLD, window.innerWidth * DISTANCE_THRESHOLD_RATIO);
};

const appendMotionSample = (pointer: PointerState, x: number, timestamp: number) => {
...
const applyDragFrame = (pointer: PointerState, clientX: number, timestamp: number) => {
...
const scheduleDragFrame = (pointerId: number, clientX: number, timestamp: number) => {
  ...
  if (typeof window === "undefined") {
    ...
    if (pointer && pointer.pointerId === pointerId && swipePhase.value === "dragging") {
...
const resolveVelocityX = (pointer: PointerState, endX: number, endTimestamp: number): number => {
  const samples = [...pointer.motionSamples, { x: endX, timestamp: endTimestamp }].sort(
  ...
  return ((endX - earliestWindowSample.x) / timeDelta) * MILLISECONDS_PER_SECOND;
};

const emitSwipePreview = (
  intensity: number,
  phase: SwipePhase = swipePhase.value,
  pivotViewportY: number | null = previewPivotViewportY.value,
  pivotCorner: PRDiscoveryCardSwipePreviewPivotCorner | null = previewPivotCorner.value,
) => {
  if (props.preview) {
    return;
  }
  ...
  emit(
    "swipe-preview",
    createPRDiscoveryCardSwipePreviewState(intensity, phase, pivotViewportY, pivotCorner),
  );
};
...
const runHintWobbleStep = (runId: number, index: number) => {
  ...
  if (typeof intensity !== "number") {
    isHintWobbling.value = false;
    settleTransition.value = PR_DISCOVERY_CARD_REBOUND_TRANSITION;
    return;
  }
...
const playHintWobble = () => {
  ...
  previewPivotViewportY.value = null;
  previewPivotCorner.value = null;
...
const startExit = (action: SwipeAction) => {
  ...
  hasDispatchedExitAction.value = false;
  swipePhase.value = "exiting";
  settleTransition.value = PR_DISCOVERY_CARD_EXIT_TRANSITION;
...
const handlePointerDown = (event: PointerEvent) => {
  ...
  const normalizedY = rect.height > 0 ? Math.min(Math.max(localY / rect.height, 0), 1) : 0.5;
  const pivotCorner: PRDiscoveryCardSwipePreviewPivotCorner = normalizedY <= 0.5 ? "top" : "bottom";
  ...
  previewPivotViewportY.value = event.clientY;
  previewPivotCorner.value = pivotCorner;
  tapDetailEligible.value = true;
  emitSwipePreview(0, "dragging", event.clientY, pivotCorner);
...
const handlePointerMove = (event: PointerEvent) => {
  const pointer = activePointer.value;
  if (!pointer || pointer.pointerId !== event.pointerId || swipePhase.value !== "dragging") {
    return;
  }
...
const resolveSwipeAction = (offsetX: number, velocityX: number): SwipeAction | null => {
...
.pr-discovery-demand-card--interactive {
  cursor: pointer;
}

.pr-discovery-demand-card--interactive:focus-visible {
  outline: 2px solid var(--sys-color-primary);
  outline-offset: 3px;
}

.pr-discovery-demand-card--pending {
  opacity: 0.9;
}

.pr-discovery-demand-card--preview {
  box-shadow: var(--sys-shadow-2);
}

.pr-discovery-demand-card--preview .pr-discovery-demand-card__cover {
  filter: saturate(0.85) brightness(0.9);
}

.pr-discovery-demand-card--preview .pr-discovery-demand-card__body {
  gap: var(--sys-spacing-small);
}

.pr-discovery-demand-card__cover {
  position: relative;
...
.pr-discovery-demand-card__cover--image {
...
.pr-discovery-demand-card__cover--fallback {
  align-items: center;
...
.pr-discovery-demand-card__fallback-location {
...
.pr-discovery-demand-card__location-tag {
  position: relative;
  z-index: 1;
}

.pr-discovery-demand-card__body {
...
.pr-discovery-demand-card__primary {
  display: flex;
  flex-direction: column;
  gap: var(--sys-spacing-xsmall);
}

.pr-discovery-demand-card__time {
  @include mx.pu-font(section);
  margin: 0;
  color: var(--sys-color-on-surface);
}

.pr-discovery-demand-card__notes {
