          ...
          >
            <PuChipGroup class="preference-custom-field__chips" wrap gap="sm">
              ...
              <PuChipInput
                ...
                commit-on-blur
                :removable="false"
                :placeholder="t('prDiscovery.customTagPlaceholder')"
...
import { useSubmitPRAuthoringPreferenceTags } from "@/domains/pr/queries/usePRAuthoringPreferenceTags";

type PRDiscoveryPresetTag = PRAuthoringOptions["preferenceTags"][number] & { id?: number };
type PreferenceCell =
  | {
      key: string;
      kind: "category";
...
const props = defineProps<{
  ...
  presetTags: readonly PRDiscoveryPresetTag[];
}>();
...
const { t } = useI18n();
const preferenceTagSubmissionMutation = useSubmitPRAuthoringPreferenceTags();

const localCustomTags = ref<PRDiscoveryPresetTag[]>([]);
...
const drawerCustomTags = ref<PRDiscoveryPresetTag[]>([]);
const customTagDraft = ref("");
const preferenceSubmissionMessage = ref<string | null>(null);

const normalizeTagKey = (label: string): string => label.trim().toLocaleLowerCase("zh-CN");

const mergeTagsByLabel = (tags: readonly PRDiscoveryPresetTag[]): PRDiscoveryPresetTag[] => {
  const byLabel = new Map<string, PRDiscoveryPresetTag>();
...
const effectivePresetTags = computed<PRDiscoveryPresetTag[]>(() =>
  mergeTagsByLabel([...props.presetTags, ...localCustomTags.value]),
);

const drawerEffectiveTags = computed<PRDiscoveryPresetTag[]>(() =>
...
const preferenceTagGroups = computed(() => buildPreferenceTagGroups(effectivePresetTags.value));

const drawerPresetTagGroups = computed(() => buildPreferenceTagGroups(props.presetTags));
    ...
    return [
      {
        key: "all",
        kind: "all",
        title: t("prDiscovery.preferencePlaceholder"),
      },
    ];
  }
...
const preferenceDrawerTitle = computed(() => {
  ...
  if (!cell || cell.kind === "all") {
    return t("prDiscovery.preferenceDrawerTitle");
  }

  return t("prDiscovery.preferenceCategoryDrawerTitle", {
    category: cell.title,
  });
});

const isTagInPreferenceCell = (tag: PRDiscoveryPresetTag, cell: PreferenceCell): boolean => {
...
const activeDrawerPresetTags = computed<PRDiscoveryPresetTag[]>(() => {
  const cell = activeDrawerCell.value;
  if (!cell) {
    return [];
  }
  ...
  if (cell.kind === "category") {
    return groups.categorized.find((group) => group.category === cell.category)?.tags ?? [];
  }
...
const activeDrawerCustomTags = computed<PRDiscoveryPresetTag[]>(() => {
  const cell = activeDrawerCell.value;
  if (!cell) {
    return [];
  }
  ...
  return mergeTagsByLabel(drawerCustomTags.value).filter(
    (tag) =>
      !presetTagKeys.value.has(normalizeTagKey(tag.label)) && isTagInPreferenceCell(tag, cell),
  );
});
...
const activeDrawerDescription = computed(() => {
  ...
  const selectedTag = activeDrawerPresetTags.value.find((tag) => tag.label === selectedLabel);
...
const stripCategoryPrefix = (label: string, category: string): string => {
  ...
  if (labelCategory.toLocaleLowerCase("zh-CN") !== category.toLocaleLowerCase("zh-CN")) {
    return normalized;
  }
...
const buildCustomTagRemoveLabel = (label: string): string => {
  const cell = activeDrawerCell.value;
  return t("prDiscovery.removeCustomTagAction", {
...
const buildPreferenceCellValue = (cell: PreferenceCell): string => {
  ...
  const values = props.modelValue.filter((label) => derivePreferenceCategory(label) === null);
...
const handlePreferenceDrawerClose = async ({ reason }: PuDrawerCloseEvent): Promise<void> => {
  if (reason === "overlay") {
...
const handleSelectDrawerCategoryTag = (category: string, label: string) => {
  drawerSelectedCategoryMap.value = {
    ...drawerSelectedCategoryMap.value,
    [category]: drawerSelectedCategoryMap.value[category] === label ? null : label,
  };
};

const handleToggleDrawerUncategorizedTag = (label: string) => {
  if (drawerSelectedUncategorizedLabels.value.includes(label)) {
    drawerSelectedUncategorizedLabels.value = drawerSelectedUncategorizedLabels.value.filter(
      (item) => item !== label,
    );
    return;
  }
  drawerSelectedUncategorizedLabels.value = [...drawerSelectedUncategorizedLabels.value, label];
};
...
const replaceDrawerSelection = (previousLabel: string, nextLabel: string) => {
  ...
  drawerSelectedUncategorizedLabels.value = drawerSelectedUncategorizedLabels.value.map((item) =>
...
const handleSavePreferenceDrawer = async () => {
  const nextSelectedPreferences = [
    ...Object.values(drawerSelectedCategoryMap.value).filter((value): value is string =>
      Boolean(value),
    ),
    ...drawerSelectedUncategorizedLabels.value,
  ];

  const existingLocalKeys = new Set(localCustomTags.value.map((tag) => normalizeTagKey(tag.label)));
  ...
  try {
    ...
    preferenceSubmissionMessage.value = t("prDiscovery.customTagSubmitFailed");
  }
};
</script>

<style lang="scss" scoped>
.pr-discovery-preference-control,
