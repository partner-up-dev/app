...
import { type ComponentPublicInstance, type CSSProperties, computed, nextTick, ref } from "vue";

type CarouselItemId = number | string;

export type PRPeekRadioCarouselItem = {
  id: CarouselItemId;
};
...
const props = withDefaults(
  defineProps<{
    ...
    items: readonly PRPeekRadioCarouselItem[];
    disabled?: boolean;
    ariaLabel?: string;
  }>(),
  {
...
const boundIndex = (index: number): number => boundNumber(index, 0, props.items.length - 1);
...
const selectIndex = (index: number, options: { focusAfterSelect?: boolean } = {}) => {
...
const moveSelection = (delta: -1 | 1, options: { focusAfterSelect?: boolean } = {}) => {
...
const handlePointerMove = (event: PointerEvent) => {
  ...
  dragOffsetPx.value = boundNumber(gestureDeltaX.value, -MAX_DRAG_OFFSET_PX, MAX_DRAG_OFFSET_PX);
};

const finalizePointerInteraction = (pointerId: number, options: { commitSelection: boolean }) => {
...
const handlePointerUp = (event: PointerEvent) => {
  ...
  const shouldCommit =
    ...
    Math.abs(gestureDeltaX.value) > Math.abs(gestureDeltaY.value) * SWIPE_AXIS_DOMINANCE_RATIO;
...
const optionStyle = (index: number): CSSProperties => {
  ...
  const translatePercent =
    direction === 0 ? 0 : direction * (72 + Math.max(0, limitedAbsOffset - 1) * 18);
  const dragInfluence = Math.max(0.38, 1 - absOffset * 0.18);
  const translatePx = dragOffsetPx.value * dragInfluence;
  const scale = absOffset === 0 ? 1 : absOffset === 1 ? 0.92 : absOffset === 2 ? 0.84 : 0.78;
  const opacity = absOffset === 0 ? 1 : absOffset === 1 ? 0.74 : absOffset === 2 ? 0.38 : 0;
