<template>
  <section class="pr-discovery-place-control">
    <div class="pr-discovery-place-control__header">
      <h2 class="pr-discovery-place-control__title">
    ...
    >
      <template #item="{ item, selected }">
        <article
          class="place-card"
          data-testid="pr-discovery.place.option"
      ...
      <div :key="selectedPlaceCaptionKey" class="place-caption">
        ...
        >
          ...
          <PuButton
            ...
            variant="ghost"
            size="sm"
            :aria-label="t('prDiscovery.placeSelector.switchRouteDirection')"
            data-testid="pr-discovery.place.route-direction-toggle"
...
import { pickStableGalleryImage } from "@/domains/pr/model/pr-discovery-form";
import type { PRDiscoveryPlaceSelectorView } from "@/domains/pr/model/pr-discovery-place-options";
import {
  arePRDiscoveryRoutesEqual,
  buildPRDiscoveryPlaceOptions,
  clonePRDiscoveryRoute,
  type PRDiscoveryPlaceOption,
  type PRDiscoveryPlaceSelection,
  type PRDiscoveryRoutePlaceOption,
  reversePRDiscoveryRoute,
...
type PlaceSelectorView = PRDiscoveryPlaceSelectorView;

const CREATE_LOCATION_CARD_ID = "__create_location__";
const CREATE_ROUTE_CARD_ID = "__create_route__";

type LocationPlaceCardViewModel = Extract<PRDiscoveryPlaceOption, { kind: "location" }> &
  PRPeekRadioCarouselItem & {
    coverImage: string | null;
    isCreateCard?: false;
  };
type RoutePlaceCardViewModel = PRDiscoveryRoutePlaceOption &
  PRPeekRadioCarouselItem & {
    coverImage: null;
    isCreateCard?: false;
    sourceRoute: PRDiscoveryRoutePlaceOption["route"];
    isReversed: boolean;
  };
type SelectablePlaceCardViewModel = LocationPlaceCardViewModel | RoutePlaceCardViewModel;
type CreatePlaceCardViewModel = PRPeekRadioCarouselItem & {
...
type PlaceCardViewModel = SelectablePlaceCardViewModel | CreatePlaceCardViewModel;

const props = defineProps<{
  modelValue: PRDiscoveryPlaceSelection | null;
  placeSelector: PlaceSelectorView;
}>();

const emit = defineEmits<{
  "update:modelValue": [value: PRDiscoveryPlaceSelection | null];
  createLocation: [];
  createRoute: [];
}>();
...
const createRoutePlaceCard = (option: PRDiscoveryRoutePlaceOption): RoutePlaceCardViewModel => {
  const isReversed = reversedRouteCardIds.value.has(option.id);

  return {
    ...option,
    coverImage: null,
    route: isReversed ? reversePRDiscoveryRoute(option.route) : clonePRDiscoveryRoute(option.route),
...
const selectablePlaceCards = computed<SelectablePlaceCardViewModel[]>(() => {
  const options = buildPRDiscoveryPlaceOptions({
...
const createLocationCard = (): CreatePlaceCardViewModel => ({
  ...
  label: t(props.placeSelector.applyActionKey ?? "prDiscovery.placeSelector.applyLocation"),
  coverImage: null,
  isCreateCard: true,
  createKind: "location",
});
...
const createRouteCard = (): CreatePlaceCardViewModel => ({
  ...
  label: t(props.placeSelector.applyActionKey ?? "prDiscovery.placeSelector.applyRoute"),
  coverImage: null,
  isCreateCard: true,
  createKind: "route",
});
...
const selectedRoutePoints = computed<RoutePoint[]>(() => selectedRoutePlaceCard.value?.route ?? []);
...
const asPlaceCardViewModel = (value: PRPeekRadioCarouselItem) => value as PlaceCardViewModel;

const asRoutePlaceCardViewModel = (value: PRPeekRadioCarouselItem) =>
...
const toSelectedPlace = (
  ...
  return {
    kind: "route",
    route: clonePRDiscoveryRoute(card.route),
  };
};
...
const switchSelectedRouteDirection = () => {
  ...
  emit("update:modelValue", {
    ...
    route: isNextReversed
      ? reversePRDiscoveryRoute(selectedRoute.sourceRoute)
      : clonePRDiscoveryRoute(selectedRoute.sourceRoute),
  });
};

const handleCarouselKeydown = (event: KeyboardEvent) => {
  if (!isCreateCardId(activeCardId.value) || (event.key !== "Enter" && event.key !== " ")) {
    return;
  }

  const activeCard = placeCards.value.find((place) => place.id === activeCardId.value);
...
const syncFromModelValue = (value: PRDiscoveryPlaceSelection | null): boolean => {
  if (!value) {
    return false;
  }
  ...
  if (value.kind === "location") {
    const locationCard = selectablePlaceCards.value.find(
      (place) => place.kind === "location" && place.locationId === value.location,
    );
    if (!locationCard) {
      return false;
    }
  ...
  for (const card of selectablePlaceCards.value) {
    ...
    const nextReversedRouteCardIds = new Set(reversedRouteCardIds.value);
    if (arePRDiscoveryRoutesEqual(card.sourceRoute, value.route)) {
    ...
    if (arePRDiscoveryRoutesEqual(reversePRDiscoveryRoute(card.sourceRoute), value.route)) {
...
.pr-discovery-place-control {
  display: flex;
  flex-direction: column;
  gap: var(--sys-spacing-small);
}

.pr-discovery-place-control__header {
  display: flex;
  flex-direction: column;
}

.pr-discovery-place-control__title {
  margin: 0;
...
.place-card {
  position: relative;
  overflow: hidden;
  height: var(--dcs-pr-discovery-location-card-height);
