...
export const useGenerateWechatThumbHtml = () => {
  return useMutation({
    mutationFn: async (params: { prId: PRId; style?: number }): Promise<PosterHtmlResponse> => {
      ...
      if (!res.ok) {
        ...
        throw new Error(payload.error ?? i18n.global.t("errors.generateWechatThumbHtmlFailed"));
      }
