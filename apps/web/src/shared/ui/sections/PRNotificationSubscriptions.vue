...
const handleOpenSubscribeSuccess = async (kind: WeChatNotificationKind, event: unknown) => {
  await notificationSubscriptions.handleOpenSubscribeSuccess(kind, extractEventDetail(event));
};

const handleOpenSubscribeError = async (kind: WeChatNotificationKind, event: unknown) => {
  await notificationSubscriptions.handleOpenSubscribeError(kind, extractEventDetail(event));
};
