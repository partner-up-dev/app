...
export const useWeChatOfficialAccountQrCode = () => {
  ...
  const officialAccountQrCodeLoading = computed(() => officialAccountQrCodeQuery.isLoading.value);

  const officialAccountQrCodeUrl = computed(() => {
    if (officialAccountQrCodeQuery.isLoading.value || officialAccountQrCodeQuery.error.value) {
      return null;
    }
