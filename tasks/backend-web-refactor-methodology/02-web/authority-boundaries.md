...
| OAuth handoff、auth bootstrap 与 route auto-login 都作用于 browser/session lifecycle；设计上通过 pending nonce defer 形成顺序约束。 | Fact | [OAuth TDD](../../../docs/30-unit-tdd/wechat-oauth-handoff.md), [auth bootstrap](../../../apps/web/src/processes/auth/useAuthSessionBootstrap.ts), [auto-login](../../../apps/web/src/processes/wechat/route-wechat-auto-login.ts)；Phase 4 completion review tracks its stale-navigation repair. |
