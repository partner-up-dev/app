...
type FakeWeChatPayState = {
  transactions: readonly unknown[];
  refunds: readonly unknown[];
};

type FakeCaocaoState = {
  createRequestCount: number;
  orders: readonly unknown[];
  feeConfirms: readonly unknown[];
};
...
type RuntimeEffectSnapshot = {
  ...
  weChatTransactionCount: number;
  weChatRefundCount: number;
  caocaoCreateRequestCount: number;
  caocaoOrderCount: number;
  caocaoFeeConfirmCount: number;
};
...
const readExternalJson = async <T>(url: string): Promise<T> => {
...
const readRuntimeEffectSnapshot = async (): Promise<RuntimeEffectSnapshot> => {
  const { fakeCaocao, fakeWeChatPay } = getScenarioEnvironment();
  const [
    quoteRows,
    orderRows,
    billRows,
    rentalOrderRows,
    createOrderAttemptRows,
    weChatState,
    caocaoState,
  ] = await Promise.all([
    ...
    readExternalJson<FakeWeChatPayState>(`${fakeWeChatPay.origin}/__fake_wechatpay/state`),
    readExternalJson<FakeCaocaoState>(`${fakeCaocao.origin}/__fake_caocao/state`),
  ]);
  ...
  return {
    ...
    weChatTransactionCount: weChatState.transactions.length,
    weChatRefundCount: weChatState.refunds.length,
    caocaoCreateRequestCount: caocaoState.createRequestCount,
    caocaoOrderCount: caocaoState.orders.length,
    caocaoFeeConfirmCount: caocaoState.feeConfirms.length,
  };
};

scenario("commerce_rental_runtime_retirement_rejects_public_writes_without_effects", async (ctx) => {
