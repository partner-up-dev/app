...
async function createSessionCatalogContext(root) {
  try {
    ...
    console.error(
      `[intent-${AGENT}-session-catalog] failed to load skill catalog: ${formatError(error)}`,
    );
    return "";
  }
}
...
function readInstalledPackageSkillCatalog(root) {
  ...
  if (!existsSync(packageJsonPath) || !existsSync(skillsRoot)) {
    return {
      skills: [],
      packages: [],
      warnings: [`Fallback package skill scan found no installed ${packageName}`],
      conflicts: [],
    };
  }

  const packageJson = JSON.parse(readFileSync(packageJsonPath, "utf8"));
  const skill = readPackageSkill(
    packageName,
    packageRoot,
    packageJson.version,
    join(skillsRoot, "design-web"),
  );
  ...
  return {
    ...
    warnings:
      skills.length === 0 ? [`Fallback package skill scan found no ${packageName} skills`] : [],
    conflicts: [],
  };
}
...
function observationFromEvent(event) {
  ...
  const command =
    typeof toolInput === "string" ? safeCommandFromString(toolInput) : commandFromObject(toolInput);
