...
- Structured, natural-language, event-assisted, Form Mode zero-candidate auto-create, and full-capacity auto-expansion PR creation converge on the same backend PR create authority after adapter-specific input preparation. Admin PR create remains a separate operator command surface.
