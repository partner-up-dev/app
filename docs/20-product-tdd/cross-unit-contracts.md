...
- event-assisted create may carry `routePoolEntryId` when the selected Anchor Event place source is a route-pool entry; backend resolves that id against the event-owned `routePool`, writes the selected entry's route into `PartnerRequest.route`, and writes `PartnerRequest.location = null`
...
- Frontend owns only route and page placement, thread rendering, composer input, join-success subscription-modal prompting, combined community follow-up rendering, official-account prompt cooldown, and cache refresh behavior. It must not infer membership, unread-wave reset, or notification gating from stale local cache.
