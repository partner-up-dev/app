...
- `POST /api/events/:eventId/form-mode/recommendation` accepts one selected place, either one selected exact start time or a fuzzy time preference expanded against the event's available start options, and the current preference labels, then returns one matched recommendation plus an ordered candidate list. Fuzzy time preference is recommendation input only and is not persisted on `PR`. When the request has a current viewer user id, backend excludes PRs where that viewer is already an active partner (`JOINED`, `CONFIRMED`, or `ATTENDED`) before ranking recommendations. When both recommendation result lists are empty, frontend submits `POST /api/pr/new/form` with a concrete selected time window and routes to the created `/pr/:id` with `handoff=event_assisted_create` so PR detail can show the created-request notice.
