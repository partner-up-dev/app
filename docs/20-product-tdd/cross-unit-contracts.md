...
- `GET /api/events/:eventId/demand-cards` groups demand through compatibility card keys, while the underlying candidate PR set comes from visible, joinable root PR reads keyed by activity type, excluding PRs whose active partner set already includes the current viewer.
