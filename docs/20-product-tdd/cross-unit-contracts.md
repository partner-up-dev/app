...
- `GET /api/admin/ride-hailing/orders/workspace` is the admin-only RideHailing order workspace read. It returns ride-hailing order records with the base trade order, ride execution state, provider binding summary, and bill summary needed by the dedicated operator surface.
- `POST /api/admin/ride-hailing/orders/:orderId/cancel` is the admin-only RideHailing cancel command. It reuses the same backend-owned ride-hailing termination admissibility as user-side cancellation for cancellable phases, but it does not require the operator to be the order creator. Frontend must show an explicit destructive confirmation before sending this command and refresh the RideHailing order workspace after success.
...
- Frontend Admin RideHailing management exposes separate provider-instance and order-admin views. Provider Instance uses the existing ride-hailing provider-instance admin endpoints. Order Admin uses the dedicated ride-hailing order workspace read plus the admin ride-hailing cancel command and stays route-local to the RideHailing admin group.
