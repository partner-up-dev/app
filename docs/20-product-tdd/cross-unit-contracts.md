...
- `GET /api/events/:eventId/demand-cards` groups demand through compatibility card keys, while the underlying candidate PR set comes from visible, joinable root PR reads whose `PR.type` resolves to that Anchor Event, excluding PRs whose active partner set already includes the current viewer.
