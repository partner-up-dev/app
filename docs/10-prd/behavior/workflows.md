...
18. If the current local date, time-pool rule, or selected place needs a new PR and the Anchor Event PR creation policy allows user creation, the user can create one through the controlled event landing flow. Card and list creation pickers include event-authored time-window description copy in each described time option.
19. The event landing route resolves its assisted-create choices into the same structured PR create payload shape used by `/pr/new`. Assisted-create place options may resolve to a primary location or to `PR.route`, and may carry transient event referral context for browser-route continuity.
