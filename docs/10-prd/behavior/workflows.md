...
18. If the current local date, time-pool rule, or selected place needs a new PR and the Anchor Event PR creation policy allows user creation, the user can create one through the controlled event-page flow. Card and list creation pickers include event-authored time-window description copy in each described time option.
